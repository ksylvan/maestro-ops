...
usage() {
    cat <<EOF
Usage: $(basename "$0") <agent_id> [grace_seconds] [poll_seconds] [agent_type]
...
Examples:
  ...
  $(basename "$0") 14fcd1d2-19ee-482b-8e4a-b521aca9a7e6 60 5 codex
EOF
}
...
# _pids_by_cwd — Match an agent by its unique worktree cwd (best effort).
# Excludes this watcher and its own awk, so the matcher never counts itself.
# Returns with no output when the cwd is unknown.
_pids_by_cwd() {
...
pids() {
    case "$agent_type" in
        claude|claude-code|"")
            # "claude" is the default type. Older CLI metadata can still say
            # "claude-code". Both run headless as `claude --print`, keyed by
            # the agent id.
        ...
        grok|hermes)
            ...
            # cwd is the stable key, as with codex but without the codex/exec
            # tokens.
            _pids_by_cwd
            ;;
        *)
            # Best-effort fallback for any future Maestro agent type.
            _pids_by_cwd
            ;;
    esac
}
